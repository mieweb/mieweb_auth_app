...
async function startServer() {
  ...
  https
    .get(
      `https://api-magic-box.dev.bluehive.com/register-dev-system?yubiKey=${yubi}&port=${availablePort}&customHandle=mieauth`,
      (res) => {
        res.on("data", (d) => {
        ...
        res.on("end", () => {
          ...
          execSync(
            `meteor run ${args.join(" ")} --settings settings.json --port ${availablePort}`,
            {
              stdio: "inherit",
            },
          );

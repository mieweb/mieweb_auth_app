...
Meteor.methods({
   ...
   */
  'pendingResponses.create': async function(username, requestId, timeoutMs = 25000) {
    ...
    console.log(`Created pending response for ${username} with ID ${requestId}, expires at ${expiresAt}`);
    return requestId;
  },
  ...
  'pendingResponses.resolve': async function(username, action) {
    ...
    console.log(`Attempting to resolve pending response for ${username} with action: ${action}`);
   ...
   */
  'pendingResponses.waitForResponse': async function(username, requestId, timeoutMs = 25000) {
    check(username, String);
    check(requestId, String);
    check(timeoutMs, Number);
    ...
    return new Promise((resolve) => {
      const pollForResponse = async () => {
        try {
          ...
          if (response) {
            console.log(`Response received for ${username}: ${response.action}`);

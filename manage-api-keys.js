...
async function generateApiKey(clientId) {
  ...
  try {
    ...
    if (existing) {
      ...
      console.log(
        'Use "regenerate" command to create a new key for this client',
      );
      return false;
    }
...
async function listApiKeys() {
  ...
  try {
    ...
    keys.forEach((key) => {
      ...
      console.log(key.clientId.padEnd(40), created.padEnd(25), lastUsed);
    });
...
async function deleteApiKey(clientId) {
  ...
  try {
    ...
    const answer = readline.question(
      `Are you sure you want to delete the API key for "${clientId}"? (yes/no): `,
    );
...
async function regenerateApiKey(clientId) {
  ...
  try {
    ...
    const answer = readline.question(
      `Are you sure you want to regenerate the API key for "${clientId}"? The old key will stop working. (yes/no): `,
    );
    if (answer.toLowerCase() !== "yes") {
  ...
  try {
    switch (command) {
      ...
      default:
        ...
        console.log(
          "  MONGO_URL - MongoDB connection URL (default: mongodb://localhost:3001/meteor)",
        );
        console.log(
          '  SEND_NOTIFICATION_FORCE_AUTH - Set to "true" to require API key authentication',
        );
        success = false;
    }
  } catch (error) {

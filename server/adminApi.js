...
WebApp.connectHandlers.use('/api/admin/api-keys/delete', async (req, res) => {
  setCors(res);
  if (req.method === 'OPTIONS') { res.writeHead(200); res.end(); return; }
  if (req.method !== 'DELETE' && req.method !== 'POST') return sendJson(res, 405, { error: 'Method not allowed' });
  ...
  await requireAdminAuth(req, res, async () => {
    try {
      ...
      await DeviceDetails.updateAsync(
        ...
        { $set: { 'devices.$.deviceRegistrationStatus': 'approved', 'devices.$.lastUpdated': new Date() } }
      );
...
WebApp.connectHandlers.use('/api/admin/emails/list', async (req, res) => {
  ...
  await requireAdminAuth(req, res, async () => {
    try {
      const emails = await EmailLog.find({}, { sort: { createdAt: -1 }, limit: 200 }).fetchAsync();

      // Enrich registration_approval emails with the user's current registration status
      const enriched = await Promise.all(emails.map(async (e) => {
        const doc = { ...e };
        if (e.type === 'registration_approval' && e.userId) {
          const user = await Meteor.users.findOneAsync({ _id: e.userId }, { fields: { 'profile.registrationStatus': 1 } });
          doc.registrationStatus = user?.profile?.registrationStatus || 'deleted';
        }
        return doc;
      }));

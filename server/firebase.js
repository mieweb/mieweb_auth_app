  ...
  try {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.ROOT_URL}/send-notification`,
        {
          method: "POST",
          headers: {
      ...
      console.log(
        `Device approval notification sent. User action: ${result.action}`,
      );
    ...
    console.log(
      `Device approval notification sent to user ${userId} for device ${newDeviceUUID}`,
    );
  } catch (error) {
    console.error("Error sending device approval notification:", error);
    throw error;

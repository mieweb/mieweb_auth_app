...
const MONGO_URL = process.env.MONGO_URL || process.env.MONGODB_URI || 'mongodb://localhost:27017/meteor';
...
async function cleanupExistingData(db) {
  ...
  try {
    ...
    console.log(`✅ Cleaned up ${result.deletedCount} existing pending responses`);
  } catch (error) {
...
async function showCollectionStats(db) {
  ...
  try {
    ...
    console.log(`📊 Average document size: ${Math.round((stats.avgObjSize || 0) / 1024 * 100) / 100} KB`);
    console.log(`📊 Total collection size: ${Math.round((stats.size || 0) / 1024 / 1024 * 100) / 100} MB`);
